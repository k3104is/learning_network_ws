...
# tcpdump to check ip on the wan/lan
...
# launch server on the lan
sudo ip netns exec lan nc -lnv 54321 &
sleep 1

# connect server from client on the wan(dst NAT)
echo 'Hello, World' | sudo ip netns exec wan nc 203.0.113.254 54321 &
sleep 1
